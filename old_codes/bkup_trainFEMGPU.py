...
wandb.init(project="hidden_nodemb_128")

#Defining Pytorch geometric graph

def graphgen(gdta,gcoord,glabel):
...
with open("/home/dami/Inverse_GNN/solution_data","rb") as fd:
    data = pickle.load(fd)

with open("/home/dami/Inverse_GNN/coordinate_data","rb") as fe:
    coord = pickle.load(fe)

with open("/home/dami/Inverse_GNN/input_flux","rb") as ff:
    flux = pickle.load(ff)
...
def train():
    ...
    for data in trainloader:
        ...
        """
        print("Batch number",c)

        #Save output and FEM solver
        np.save("GNN_output.npy",out.cpu().detach().numpy())
        os.system("python FEM.py > FEMtextout.txt")

    
        with open("/home/dami/Inverse_GNN/FEM_output/solution_data_OPT","rb") as fd:
            fdata = pickle.load(fd)

        with open("/home/dami/Inverse_GNN/FEM_output/coordinate_data_OPT","rb") as fe:
            coord = pickle.load(fe)

        with open("/home/dami/Inverse_GNN/FEM_output/input_flux_OPT","rb") as ff:
            flux = pickle.load(ff)
        ...
        loss = criterion( nFEMout.cpu(), ndata.cpu() ) #Loss is between input solution mesh and output of FEM solver
...
def validate():
    ...
    for data in vadloader:
        ...
        nFEMout = Variable(FEMout, requires_grad=True).to(torch.double)
        ndata = data.x.squeeze().to(torch.double)
        loss = criterion( nFEMout, ndata.cpu() ) #Loss is between input solution mesh and output of FEM solver
        """
...
for data in testloader:
    ...
    wandb.log({"coefficinets":wandb.Table(data=coefdata,columns=["predictions","labels"])})
